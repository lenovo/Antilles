...
window.elFinder = function(node, opts) {
	...
	var self = this,
		 ...
		 * @type jQuery
		 **/
		prevContent = $jq('<div/>').append(node.contents()),
		...
		beeper = $jq(document.createElement('audio')).hide().appendTo('body')[0],
		...
		execShortcut = function(e) {
			...
			if (enabled) {
				...
				$jq.each(shortcuts, function(i, shortcut) {
				...
				if (code == 9 && !$jq(e.target).is(':input')) {
					e.preventDefault();
				}
	 ...
	 * @type Object
	 **/
	this.options = $jq.extend(true, {}, this._options, opts||{});
	...
	$jq.extend(this.options.contextmenu, opts.contextmenu);
	 ...
	 * @default {}
	 **/
	this.customData = $jq.isPlainObject(this.options.customData) ? this.options.customData : {};
	...
	i18n = this.lang == 'en'
		? this.i18['en']
		: $jq.extend(true, {}, this.i18['en'], this.i18[this.lang]);
	...
	this.sortRules = $jq.extend(true, {}, this._sortRules, this.options.sortsRules);

	$jq.each(this.sortRules, function(name, method) {
		if (typeof method != 'function') {
			delete self.sortRules[name];
		}
	});

	this.compare = $jq.proxy(this.compare, this);
	...
	this.draggable = {
		...
		start      : function(e, ui) {
			var targets = $jq.map(ui.helper.data('files')||[], function(h) { return h || null ;}),
			cnt, h;
			cnt = targets.length;
			while (cnt--) {
				h = targets[cnt];
		...
		helper     : function(e, ui) {
			var element = this.id ? $jq(this) : $jq(this).parents('[id]:first'),
				helper  = $jq('<div class="elfinder-drag-helper"><span class="elfinder-drag-helper-icon-plus"/></div>'),
			...
			$jq(document).bind(keydown + ' keyup.' + namespace, function(e){
	...
	// Disable drag function by Wengmh1 @ 2018-3-8
	this.draggable = null;
	/**
	...
	this.droppable = {
			...
			drop : function(e, ui) {
				var dst     = $jq(this),
					targets = $jq.map(ui.helper.data('files')||[], function(h) { return h || null }),
					result  = [],
					c       = 'class',
					cnt, hash, i, h;
	...
	this.url = function(hash) {
		...
		if (cwdOptions.url) {
			return cwdOptions.url + $jq.map(this.path2array(hash), function(n) { return encodeURIComponent(n); }).slice(1).join('/')
		}

		var params = $jq.extend({}, this.customData, {
			cmd: 'file',
			target: file.hash
		});
		if (this.oldAPI) {
		...
		return this.options.url + (this.options.url.indexOf('?') === -1 ? '?' : '&') + $jq.param(params, true);
	}
	...
	this.selectedFiles = function() {
		return $jq.map(selected, function(hash) { return files[hash] ? $jq.extend({}, files[hash]) : null });
	};
	...
	this.request = function(options) {
		var self     = this,
			...
			dfrd     = $jq.Deferred(),
			// request data
			data     = $jq.extend({}, o.customData, {mimes : o.onlyMimes}, options.data || options),
			// command name
			cmd      = data.cmd,
			...
			notify   = $jq.extend({}, options.notify),
			 ...
			 **/
				...
				cmd == 'open' && open($jq.extend(true, {}, data));
			...
			success = function(response) {
				...
				if(!response.type) {
					...
					} else if (!$jq.isPlainObject(response)) {
					...
					if (response.options) {
						cwdOptions = $jq.extend({}, cwdOptions, response.options);
					}
	...
	this.diff = function(incoming) {
		...
		$jq.each(incoming, function(i, f) {
			raw[f.hash] = f;
		});

		// find removed
		$jq.each(files, function(hash, f) {
			!raw[hash] && removed.push(hash);
		});

		// compare files
		$jq.each(raw, function(hash, file) {
			var origin = files[hash];

			if (!origin) {
				added.push(file);
			} else {
				$jq.each(file, function(prop) {
		...
		$jq.each(removed, function(i, hash) {
			var file  = files[hash],
				phash = file.phash;

			if (phash
			&& file.mime == 'directory'
			&& $jq.inArray(phash, removed) === -1
			&& raw[phash]
			&& !isChanged(phash)) {
	...
	this.sync = function() {
		var self  = this,
			dfrd  = $jq.Deferred().done(function() { self.trigger('sync'); }),
			opts1 = {
	...
	this.trigger = function(event, data) {
		...
		if (handlers.length) {
			...
			for (i = 0; i < handlers.length; i++) {
				...
				event.data = $jq.extend(true, {}, data);
	...
	this.shortcut = function(s) {
		...
		if (this.options.allowShortcuts && s.pattern && $jq.isFunction(s.callback)) {
			...
			for (i= 0; i < patterns.length; i++) {
				...
				if (code && !shortcuts[pattern]) {
					shortcuts[pattern] = {
						keyCode     : code,
						altKey      : $jq.inArray('ALT', parts)   != -1,
						ctrlKey     : $jq.inArray('CTRL', parts)  != -1,
						shiftKey    : $jq.inArray('SHIFT', parts) != -1,
						type        : s.type || 'keydown',
						callback    : s.callback,
	...
	this.shortcuts = function() {
		...
		$jq.each(shortcuts, function(i, s) {
	...
	this.clipboard = function(hashes, cut) {
		var map = function() { return $jq.map(clipboard, function(f) { return f.hash }); }

		if (hashes !== void(0)) {
			clipboard.length && this.trigger('unlockfiles', {files : map()});
			remember = [];

			clipboard = $jq.map(hashes||[], function(hash) {
				var file = files[hash];
				if (file) {
	...
	this.isCommandEnabled = function(name) {
		return this._commands[name] ? $jq.inArray(name, cwdOptions.disabled) === -1 : false;
	}
	...
	this.exec = function(cmd, files, opts) {
		return this._commands[cmd] && this.isCommandEnabled(cmd)
			...
			: $jq.Deferred().reject('No such command');
	}
	...
	this.dialog = function(content, options) {
		return $jq('<div/>').append(content).appendTo(node).elfinderdialog(options);
	}
	...
	this.destroy = function() {
		if (node && node[0].elfinder) {
			...
			listeners = {};
			shortcuts = {};
			$jq(document).add(node).unbind('.'+this.namespace);
	...
	// check jquery ui
	if (!($jq.fn.selectable && $jq.fn.draggable && $jq.fn.droppable)) {
	...
	if (typeof(this.transport.send) != 'function') {
		this.transport.send = function(opts) { return $jq.ajax(opts); }
	}

	if (this.transport.upload == 'iframe') {
		this.transport.upload = $jq.proxy(this.uploads.iframe, this);
	} else if (typeof(this.transport.upload) == 'function') {
		this.dragUpload = !!this.options.dragUploadAllow;
	} else if (this.xhrUpload && !!this.options.dragUploadAllow) {
		this.transport.upload = $jq.proxy(this.uploads.xhr, this);
	...
	$jq.each(['enable', 'disable', 'load', 'open', 'reload', 'select',  'add', 'remove', 'change', 'dblclick', 'getfile', 'lockfiles', 'unlockfiles', 'dragstart', 'dragstop', 'search', 'searchend', 'viewchange'], function(i, name) {
		self[name] = function() {
			var arg = arguments[0];
			return arguments.length == 1 && typeof(arg) == 'function'
				? self.bind(name, arg)
				: self.trigger(name, $jq.isPlainObject(arg) ? arg : {});
		}
	});
	...
	this
		.enable(function() {
			if (!enabled && self.visible() && self.ui.overlay.is(':hidden')) {
				enabled = true;
				$jq('texarea:focus,input:focus,button').blur();
		...
		.select(function(e) {
			selected = $jq.map(e.data.selected || e.data.value|| [], function(hash) { return files[hash] ? hash : null; });
		})
		.error(function(e) {
			var opts  = {
			...
			opts.buttons[self.i18n(self.i18n('btnClose'))] = function() { $jq(this).elfinderdialog('close'); };
		...
		.bind('tmb', function(e) {
			$jq.each(e.data.images||[], function(hash, tmb) {
				if (files[hash]) {
					files[hash].tmb = tmb;
				}
			})
		...
		.bind('rm', function(e) {
			...
			//play && play != '' && play != 'no' && $jq(beeper).html('<source src="js/libs/elfinder/sounds/rm.mp3" type="audio/wav">')[0].play()
		})

		;

	// bind external event handlers
	$jq.each(this.options.handlers, function(event, callback) {
	...
	if (!$jq.isArray(this.options.commands)) {
		this.options.commands = [];
	}
	// check required commands
	$jq.each(['open', 'reload', 'back', 'forward', 'up', 'home', 'info', 'quicklook', 'getfile', 'help'], function(i, cmd) {
		$jq.inArray(cmd, self.options.commands) === -1 && self.options.commands.push(cmd);
	});

	// load commands
	$jq.each(this.options.commands, function(i, name) {
		var cmd = self.commands[name];
		if ($jq.isFunction(cmd) && !self._commands[name]) {
			cmd.prototype = base;
	...
	this.ui = {
		...
		workzone : $jq('<div/>').appendTo(node).elfinderworkzone(this),
		// container for folders tree / places
		navbar : $jq('<div/>').appendTo(node).elfindernavbar(this, this.options.uiOptions.navbar || {}),
		// contextmenu
		contextmenu : $jq('<div/>').appendTo(node).elfindercontextmenu(this),
		// overlay
		overlay : $jq('<div/>').appendTo(node).elfinderoverlay({
			show : function() { self.disable(); },
			hide : function() { prevEnabled && self.enable(); }
		}),
		// current folder container
		cwd : $jq('<div/>').appendTo(node).elfindercwd(this, this.options.uiOptions.cwd || {}),
		...
		statusbar : $jq('<div class="ui-widget-header ui-helper-clearfix ui-corner-bottom elfinder-statusbar"/>').hide().appendTo(node)
	}

	// load required ui
	$jq.each(this.options.ui || [], function(i, ui) {
		var name = 'elfinder'+ui,
			opts = self.options.uiOptions[ui] || {};

		if (!self.ui[ui] && $jq.fn[name]) {
			self.ui[ui] = $jq('<'+(opts.tag || 'div')+'/>').appendTo(node)[name](self, opts);
		}
	});
	...
	$jq(document)
		...
		.bind('click.'+this.namespace, function(e) { enabled && !$jq(e.target).closest(node).length && self.disable(); })
		// exec shortcuts
		.bind(keydown+' '+keypress, execShortcut);

	// attach events to window
	self.options.useBrowserHistory && $jq(window)
		.on('popstate', function(ev) {
			var target = ev.originalEvent.state && ev.originalEvent.state.thash;
			target && !$jq.isEmptyObject(self.files()) && self.request({
...
elFinder.prototype = {
	...
	rules : {
		defaults : function(data) {
			if (!data
			|| (data.added && !$jq.isArray(data.added))
			||  (data.removed && !$jq.isArray(data.removed))
			||  (data.changed && !$jq.isArray(data.changed))) {
				return false;
			}
			return true;
		},
		open    : function(data) { return data && data.cwd && data.files && $jq.isPlainObject(data.cwd) && $jq.isArray(data.files); },
		tree    : function(data) { return data && data.tree && $jq.isArray(data.tree); },
		parents : function(data) { return data && data.tree && $jq.isArray(data.tree); },
		tmb     : function(data) { return data && data.images && ($jq.isPlainObject(data.images) || $jq.isArray(data.images)); },
		upload  : function(data) { return data && ($jq.isPlainObject(data.added) || $jq.isArray(data.added));},
		search  : function(data) { return data && data.files && $jq.isArray(data.files)}
	},
	...
	parseUploadData : function(text) {
		...
		data.removed = $jq.map(data.added||[], function(f) { return f.hash; })
		return data;
	...
	uploads : {
		...
		iframe : function(data, fm) {
			var self   = fm ? fm : this,
				...
				form = $jq('<form action="'+self.uploadURL+'" method="post" enctype="multipart/form-data" encoding="multipart/form-data" target="'+name+'" style="display:none"><input type="hidden" name="cmd" value="upload" /></form>'),
				msie = this.UA.IE,
				...
				onload = function() {
					...
					setTimeout(function() {
						msie && $jq('<iframe src="javascript:false;"/>').appendTo(form);
						form.remove();
						iframe.remove();
					}, 100);
				},
				iframe = $jq('<iframe src="'+(msie ? 'javascript:false;' : 'about:blank')+'" name="'+name+'" style="position:absolute;left:-1000px;top:-1000px" />')
					.bind('load', function() {
						iframe.unbind('load')
							.bind('load', function() {
			...
			if (input && $jq(input).is(':file') && $jq(input).val()) {
				form.append(input);
			} else {
				return dfrd.reject();
			}
			...
			form.append('<input type="hidden" name="'+(self.newAPI ? 'target' : 'current')+'" value="'+self.cwd().hash+'"/>')
				...
				.append($jq(input).attr('name', 'upload[]'));

			$jq.each(self.options.onlyMimes||[], function(i, mime) {
				form.append('<input type="hidden" name="mimes[]" value="'+mime+'"/>');
			});

			$jq.each(self.options.customData, function(key, val) {
		...
		xhr : function(data, fm) {
			...
			// Add by Wengmh1
			xhr.setRequestHeader('authorization', 'Jwt ' + window.gApp.$store.state.auth.token);
	...
	one : function(event, callback) {
		var self = this,
			h    = $jq.proxy(callback, function(event) {
	...
	cookie : function(name, value) {
		...
		o = $jq.extend({}, this.options.cookie);
		if (value === null) {
			value = '';
			o.expires = -1;
		}
	...
	normalize : function(data) {
		...
		if (data.files) {
			data.files = $jq.map(data.files, filter);
		}
		if (data.tree) {
			data.tree = $jq.map(data.tree, filter);
		}
		if (data.added) {
			data.added = $jq.map(data.added, filter);
		}
		if (data.changed) {
			data.changed = $jq.map(data.changed, filter);
		}
		if (data.api) {
			data.init = true;
		}
	...
	notify : function(opts) {
		...
		if (!notify.length) {
			notify = $jq(ntpl.replace(/\{type\}/g, type).replace(/\{msg\}/g, msg))
				.appendTo(ndialog)
				.data('cnt', 0);
	...
	confirm : function(opts) {
		var complete = false,
			options = {
				...
				close     : function() {
					...
					$jq(this).elfinderdialog('destroy');
				}
			},
		...
		if (opts.all) {
			...
			options.create = function() {
				checkbox = $jq('<input type="checkbox" />');
				$jq(this).next().children().before($jq('<label>'+apply+'</label>').prepend(checkbox));
			}

			options.open = function() {
				var pane = $jq(this).next(),
					width = parseInt(pane.children(':first').outerWidth() + pane.children(':last').outerWidth());

				if (width > parseInt(pane.width())) {
					$jq(this).closest('.elfinder-dialog').width(width+30);
				}
			}
		}
	...
	i18n : function() {
		...
		return $jq.map(input, function(m, i) { return $jq.inArray(i, ignore) === -1 ? m : null; }).join('<br>');
	},
	...
	debug : function(type, m) {
		...
		if (d == 'all' || d === true || ($jq.isArray(d) && $jq.inArray(type, d) != -1)) {
...
elFinder.prototype._options = {
	 ...
	 * transport.send must return $jq.Deferred() object
	 *
	 * @type Object
	 * @default null
	 * @example
...
elFinder.prototype.command = function(fm) {
	...
	this.setup = function(name, opts) {
		...
		this.options   = $jq.extend({}, this.options, opts);
		this.listeners = [];
		...
		$jq.each($jq.extend({}, self._handlers, self.handlers), function(cmd, handler) {
			fm.bind(cmd, $jq.proxy(handler, self));
		});

		for (i = 0; i < this.shortcuts.length; i++) {
			s = this.shortcuts[i];
			s.callback = $jq.proxy(s.callback || function() { this.exec() }, this);
	...
	this.hashes = function(hashes) {
		return hashes
			? $jq.map($jq.isArray(hashes) ? hashes : [hashes], function(hash) { return fm.file(hash) ? hash : null; })
			: fm.selected();
	}
	...
	this.files = function(hashes) {
		...
		return hashes
			? $jq.map($jq.isArray(hashes) ? hashes : [hashes], function(hash) { return fm.file(hash) || null })
			: fm.selectedFiles();
	}
}
	...
	mixin : {
		make : function() {
			var fm   = this.fm,
				...
				input = $jq('<input type="text"/>')
					.keydown(function(e) {
						e.stopImmediatePropagation();

						if (e.keyCode == $jq.ui.keyCode.ESCAPE) {
							dfrd.reject();
						} else if (e.keyCode == $jq.ui.keyCode.ENTER) {
							input.blur();
						}
					})
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.dialogelfinder = function(opts) {
	...
	this.not('.elfinder').each(function() {
		...
		var doc     = $jq(document),
			toolbar = $jq('<div class="ui-widget-header dialogelfinder-drag ui-corner-top">'+(opts.title || 'Files')+'</div>'),
			button  = $jq('<a href="#" class="dialogelfinder-drag-close ui-corner-all"><span class="ui-icon ui-icon-closethick"/></a>')
				.appendTo(toolbar)
				.click(function(e) {
					e.preventDefault();
			...
			node    = $jq(this).addClass('dialogelfinder')
		...
		opts.autoOpen !== false && $jq(this).dialogelfinder('open');
	...
	if (opts == 'open') {
		var node = $jq(this),
			pos  = node.data(position) || {
				top  : parseInt($jq(document).scrollTop() + ($jq(window).height() < node.height() ? 2 : ($jq(window).height() - node.height())/2)),
				left : parseInt($jq(document).scrollLeft() + ($jq(window).width() < node.width()  ? 2 : ($jq(window).width()  - node.width())/2))
			},
			zindex = 100;

		if (node.is(':hidden')) {

			$jq('body').find(':visible').each(function() {
				var $this = $jq(this), z;
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfinderbutton = function(cmd) {
	return this.each(function() {
		...
		var c        = 'class',
			...
			menu,
			button   = $jq(this).addClass('ui-state-default elfinder-button')
		...
		// if command has variants create menu
		if ($jq.isArray(cmd.variants)) {
			button.addClass('elfinder-menubutton');

			menu = $jq('<div class="ui-widget ui-widget-content elfinder-button-menu ui-corner-all"/>')
				.hide()
				.appendTo(button)
				.zIndex(12+button.zIndex())
				.delegate('.'+item, 'hover', function() { $jq(this).toggleClass(hover) })
				.delegate('.'+item, 'click', function(e) {
					e.preventDefault();
					e.stopPropagation();
					button.removeClass(hover);
					cmd.exec(cmd.fm.selected(), $jq(this).data('value'));
				});
			...
			cmd.change(function() {
				menu.html('');
				$jq.each(cmd.variants, function(i, variant) {
					menu.append($jq('<div class="'+item+'">'+variant[1]+'</div>').data('value', variant[0]).addClass(variant[0] == cmd.value ? selected : ''));
				});
			});
		}
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfindercontextmenu = function(fm) {

	return this.each(function() {
		var menu = $jq(this).addClass('ui-helper-reset ui-widget ui-state-default ui-corner-all elfinder-contextmenu elfinder-contextmenu-'+fm.direction)
				.hide()
				.appendTo('body')
				.delegate('.elfinder-contextmenu-item', 'mouseenter mouseleave', function() {
					$jq(this).toggleClass('ui-state-hover')
				}),
			subpos  = fm.direction == 'ltr' ? 'left' : 'right',
			types = $jq.extend({}, fm.options.contextmenu),
			...
			item = function(label, icon, callback) {
				return $jq(tpl.replace('{icon}', icon ? 'elfinder-button-icon-'+icon : '').replace('{label}', label))
					.click(function(e) {
						e.stopPropagation();
						e.stopPropagation();
						callback();
			...
			create = function(type, targets) {
				...
				$jq.each(types[type]||[], function(i, name) {
					...
					if (cmd && cmd.getstate(targets) != -1) {
						if (cmd.variants) {
							...
							submenu = $jq('<div class="ui-corner-all elfinder-contextmenu-sub"/>')
								.appendTo(node.append('<span class="elfinder-contextmenu-arrow"/>'));

							hover = function(){
									var win    = $jq(window),
									baseleft   = $jq(node).offset().left,
									basetop    = $jq(node).offset().top,
									basewidth  = $jq(node).outerWidth(),
							...
							$jq.each(cmd.variants, function(i, variant) {
								submenu.append(
									$jq('<div class="elfinder-contextmenu-item"><span>'+variant[1]+'</span></div>')
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfindercwd = function(fm, options) {
	...
	this.not('.elfinder-cwd').each(function() {
		...
		var
			...
			selectAll = function() {
				...
				selectedFiles = $jq.map(fm.files(), function(f) { return f.phash == phash ? f.hash : null ;});
				trigger();
			},
			...
			render = function() {
				...
				if (selectedFiles.length) {
					place.find('[id]:not(.'+clSelected+'):not(.elfinder-cwd-parent)').each(function() {
						...
						$jq.inArray(id, selectedFiles) !== -1 && $jq(this).trigger(evtSelect);
					});
				}
			 ...
			 * @type Object
			 */
			droppable = $jq.extend({}, fm.droppable, {
				over : function(e, ui) {
					var hash = fm.cwd().hash;
					$jq.each(ui.helper.data('files'), function(i, h) {
			...
			attachThumbnails = function(images) {
				...
				$jq.each(images, function(hash, tmb) {
			...
			content = function(files, any) {
				...
				buffer = $jq.map(files, function(f) { return any || f.phash == phash ? f : null; });
				...
				if (options.oldSchool && phash && !query) {
					var parent = $jq.extend(true, {}, fm.file(phash), {name : '..', mime : 'directory'});
					parent = $jq(itemhtml(parent))
			...
			cwd = $jq(this)
				...
				.delegate(fileSelector, 'click.'+fm.namespace, function(e) {
					var p    = this.id ? $jq(this) : $jq(this).parents('[id]:first'),
				...
				.delegate(fileSelector, evtSelect, function(e) {
					...
					if (!selectLock && !$this.is('.'+clDisabled)) {
						...
						if ($jq.inArray(id, selectedFiles) === -1) {
				...
				.delegate(fileSelector, evtUnselect, function(e) {
					...
					if (!selectLock) {
						$jq(this).removeClass(clSelected).children().removeClass(clHover);
						ndx = $jq.inArray(id, selectedFiles);
						if (ndx !== -1) {
				...
				.delegate(fileSelector, evtDisable, function() {
					var $this  = $jq(this).removeClass(clSelected).addClass(clDisabled),
				...
				.delegate(fileSelector, evtEnable, function() {
					var $this  = $jq(this).removeClass(clDisabled),
						target = list ? $this : $this.children();
				...
				.delegate(fileSelector, 'scrolltoview', function() {
					scrollToView($jq(this));
				})
				.delegate(fileSelector, 'mouseenter.'+fm.namespace+' mouseleave.'+fm.namespace, function(e) {
					fm.trigger('hover', {hash : $jq(this).attr('id'), type : e.type});
					$jq(this).toggleClass('ui-state-hover');
				})
				.bind('contextmenu.'+fm.namespace, function(e) {
					var file = $jq(e.target).closest('.'+clFile);

					if (file.length && (e.target.nodeName != 'TD' || $jq.inArray(file.get(0).id, fm.selected()) > -1)) {
						e.stopPropagation();
						e.preventDefault();
				...
				.selectable({
					...
					stop       : trigger,
					delay      : 250,
					selected   : function(e, ui) { $jq(ui.selected).trigger(evtSelect); },
					unselected : function(e, ui) { $jq(ui.unselected).trigger(evtUnselect); }
				})
				...
				.bind('create.'+fm.namespace, function(e, file) {
					var parent = list ? cwd.find('tbody') : cwd,
						...
						file = $jq(itemhtml(file)).addClass(clTmp);
			...
			wrapper = $jq('<div class="elfinder-cwd-wrapper"/>')
				...
				.bind('touchmove.'+fm.namespace+' touchend.'+fm.namespace, function(e) {
					clearTimeout($jq(this).data('tmlongtap'));
				}),
			...
			// elfinder node
			parent = $jq(this).parent().resize(resize),
			// workzone node
			wz = parent.children('.elfinder-workzone').append(wrapper.append(this))
			;

		// for iOS5 bug
		$jq('body').on('touchstart touchmove touchend', function(e){});
		...
		fm
			...
			.add(function(e) {
				var phash = fm.cwd().hash,
					files = query
						? $jq.map(e.data.added || [], function(f) { return f.name.indexOf(query) === -1 ? null : f ;})
						: $jq.map(e.data.added || [], function(f) { return f.phash == phash ? f : null; })
						;
				add(files);
			})
			.change(function(e) {
				...
				if (query) {
					$jq.each(e.data.changed || [], function(i, file) {
						...
						if (file.name.indexOf(query) !== -1) {
							add([file]);
							$jq.inArray(file.hash, sel) !== -1 && selectFile(file.hash);
						}
					});
				} else {
					$jq.each($jq.map(e.data.changed || [], function(f) { return f.phash == phash ? f : null; }), function(i, file) {
						remove([file.hash]);
						add([file]);
						$jq.inArray(file.hash, sel) !== -1 && selectFile(file.hash);
					});
			...
			.bind('mkdir mkfile duplicate upload rename archive extract', function(e) {
				...
				$jq.each(e.data.added || [], function(i, file) {
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfinderdialog = function(opts) {
	var dialog;
	...
	opts = $jq.extend({}, $jq.fn.elfinderdialog.defaults, opts);

	this.filter(':not(.ui-dialog-content)').each(function() {
		var self       = $jq(this).addClass('ui-dialog-content ui-widget-content'),
			parent     = self.parent(),
			...
			buttonset  = $jq('<div class="ui-dialog-buttonset"/>'),
			buttonpane = $jq('<div class=" ui-helper-clearfix ui-dialog-buttonpane ui-widget-content"/>')
				.append(buttonset),

			dialog = $jq('<div class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-draggable std42-dialog  '+cldialog+' '+opts.cssClass+'"/>')
				.hide()
				.append(self)
				.appendTo(parent)
				...
				.bind('open', function() {
					dialog.trigger('totop');
					typeof(opts.open) == 'function' && $jq.proxy(opts.open, self[0])();
				...
				.bind('close', function() {
					...
					$jq(this).data('modal') && overlay.elfinderoverlay('hide');
					...
					if (typeof(opts.close) == 'function') {
						$jq.proxy(opts.close, self[0])();
					} else if (opts.destroyOnClose) {
						dialog.hide().remove();
					}
				})
				.bind('totop', function() {
					$jq(this).mousedown().find('.ui-button:first').focus().end().find(':text:first').focus();
					$jq(this).data('modal') && overlay.elfinderoverlay('show');
					overlay.zIndex($jq(this).zIndex());
				})
		...
		if (opts.closeOnEscape) {
			$jq(document).bind('keyup.'+id, function(e) {
				if (e.keyCode == $jq.ui.keyCode.ESCAPE && dialog.is('.'+clactive)) {
					self.elfinderdialog('close');
					$jq(document).unbind('keyup.'+id);
				}
			})
		}
		dialog.prepend(
			$jq('<div class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix">'+opts.title+'</div>')
				.prepend($jq('<a href="#" class="ui-dialog-titlebar-close ui-corner-all"><span class="ui-icon ui-icon-closethick"/></a>')
					.mousedown(function(e) {
						e.preventDefault();
						self.elfinderdialog('close');
					}))
		...
		$jq.each(opts.buttons, function(name, cb) {
			var button = $jq('<button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only"><span class="ui-button-text">'+name+'</span></button>')
				.click($jq.proxy(cb, self[0]))
				.hover(function(e) { $jq(this)[e.type == 'mouseenter' ? 'focus' : 'blur']() })
				.focus(function() { $jq(this).addClass(clhover) })
				.blur(function() { $jq(this).removeClass(clhover) })
				.keydown(function(e) {
					var next;

					if (e.keyCode == $jq.ui.keyCode.ENTER) {
						$jq(this).click();
					}  else if (e.keyCode == $jq.ui.keyCode.TAB) {
						next = $jq(this).next('.ui-button');
						next.length ? next.focus() : $jq(this).parent().children('.ui-button:first').focus()
					}
				})
		...
		if (opts.resizable && $jq.fn.resizable) {
			dialog.resizable({
		...
		typeof(opts.create) == 'function' && $jq.proxy(opts.create, this)();
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfindernavbar = function(fm, opts) {

	this.not('.elfinder-navbar').each(function() {
		var nav    = $jq(this).addClass('ui-state-default elfinder-navbar'),
			parent = nav.parent()
				.resize(function() {
...
$jq.fn.elfinderoverlay = function(opts) {

	this.filter(':not(.elfinder-overlay)').each(function() {
		opts = $jq.extend({}, opts);
		$jq(this).addClass('ui-widget-overlay elfinder-overlay')
			.hide()
			.mousedown(function(e) {
				e.preventDefault();
				e.stopPropagation();
...
$jq.fn.elfinderpanel = function(fm) {

	return this.each(function() {
		var panel = $jq(this).addClass('elfinder-panel ui-state-default ui-corner-all'),
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfinderpath = function(fm) {
	return this.each(function() {
		var path = $jq(this).addClass('elfinder-path').html('&nbsp;')
				.delegate('a', 'click', function(e) {
					var hash = $jq(this).attr('href').substr(1);
			...
			fm.bind('open searchend', function() {
				...
				$jq.each(fm.parents(fm.cwd().hash), function(i, hash) {
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfinderplaces = function(fm, opts) {
	return this.each(function() {
		var dirs      = [],
			c         = 'class',
			...
			spinner   = $jq(fm.res('tpl', 'navspinner')),
			/**
			 ...
			 **/
			create    = function(dir) {
				return $jq(tpl.replace(/\{id\}/, hash2id(dir.hash))
			...
			add = function(dir) {
				...
				if (subtree.children().length) {
					$jq.each(subtree.children(), function() {
						var current =  $jq(this);
				...
				node.draggable({
					...
					helper   : function() {
						...
						return $jq('<div class="elfinder-place-drag elfinder-'+fm.direction+'"/>')
								.append(dir.clone())
								.data('hash', id2hash(dir.children(':first').attr('id')));

					},
					start    : function() { $jq(this).hide(); },
					stop     : function(e, ui) {
			 ...
			 * @type jQuery
			 **/
			places = $jq(this).addClass(fm.res(c, 'tree')+' elfinder-places ui-corner-all')
				.hide()
				.append(wrapper)
				...
				.delegate('.'+navdir, 'click', function(e) {
					fm.exec('open', $jq(this).attr('id').substr(6));
				})
				.delegate('.'+navdir+':not(.'+clroot+')', 'contextmenu', function(e) {
					var hash = $jq(this).attr('id').substr(6);
				...
				.droppable({
					...
					drop       : function(e, ui) {
						...
						$jq.each(ui.helper.data('files'), function(i, hash) {
							var dir = fm.file(hash);

							if (dir && dir.mime == 'directory' && $jq.inArray(dir.hash, dirs) === -1) {
								add(dir);
							} else {
								resolve = false;
							}
		...
		fm.one('load', function() {
			...
			dirs = $jq.map((fm.storage('places') || '').split(','), function(hash) { return hash || null;});
			...
			fm.remove(function(e) {
				$jq.each(e.data.removed, function(i, hash) {
					remove(hash);
				});
				save();
			})
			.change(function(e) {
				$jq.each(e.data.changed, function(i, file) {
					if ($jq.inArray(file.hash, dirs) !== -1) {
						remove(file.hash);
						file.mime == 'directory' && add(file);
					}
				});
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfindersearchbutton = function(cmd) {
	return this.each(function() {
		var result = false,
			button = $jq(this).hide().addClass('ui-widget-content elfinder-button '+cmd.fm.res('class', 'searchbtn')+''),
			search = function() {
				var val = $jq.trim(input.val());
				if (val) {
			...
			input  = $jq('<input type="text" size="42"/>')
				.appendTo(button)
		...
		$jq('<span class="ui-icon ui-icon-search" title="'+cmd.title+'"/>')
			.appendTo(button)
			.click(search);

		$jq('<span class="ui-icon ui-icon-close"/>')
			.appendTo(button)
			.click(abort);
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfindersortbutton = function(cmd) {
	...
	return this.each(function() {
		var fm       = cmd.fm,
			...
			asc      = selected+'-asc',
			desc     = selected+'-desc',
			button   = $jq(this).addClass('ui-state-default elfinder-button elfinder-menubutton elfiner-button-'+name)
				.attr('title', cmd.title)
		...
		$jq.each(fm.sortRules, function(name, value) {
			menu.append($jq('<div class="'+item+'" rel="'+name+'"><span class="ui-icon ui-icon-arrowthick-1-n"/><span class="ui-icon ui-icon-arrowthick-1-s"/>'+fm.i18n('sort'+name)+'</div>').data('type', name));
		});

		menu.children().click(function(e) {
			var type = $jq(this).attr('rel');
		...
		$jq('<div class="'+item+' '+item+'-separated"><span class="ui-icon ui-icon-check"/>'+fm.i18n('sortFoldersFirst')+'</div>')
			.appendTo(menu)
			.click(function() {
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfinderstat = function(fm) {
	return this.each(function() {
		var size       = $jq(this).addClass('elfinder-stat-size'),
			sel        = $jq('<div class="elfinder-stat-selected"/>'),
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfindertoolbar = function(fm, opts) {
	this.not('.elfinder-toolbar').each(function() {
		var commands = fm._commands,
			self     = $jq(this).addClass('ui-helper-clearfix ui-widget-header ui-corner-top elfinder-toolbar'),
			panels   = opts || [],
			l        = panels.length,
			i, cmd, panel, button;
		...
		while (l--) {
			if (panels[l]) {
				panel = $jq('<div class="ui-widget-content ui-corner-all elfinder-buttonset"/>');
				i = panels[l].length;
				while (i--) {
					if ((cmd = commands[panels[l][i]])) {
						button = 'elfinder'+cmd.options.ui;
						$jq.fn[button] && panel.prepend($jq('<div/>')[button](cmd));
					}
				}
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfindertree = function(fm, opts) {
	...
	this.not('.'+treeclass).each(function() {
		...
		var c = 'class',
			 ...
			 * @type Object
			 */
			droppableopts = $jq.extend(true, {}, fm.droppable, {
				...
				out : function() { $jq(this).removeClass(hover+' '+dropover); },
			...
			filter = function(files) {
				return $jq.map(files||[], function(f) { return f.mime == 'directory' ? f : null });
			},
			...
			updateArrows = function(dirs, cls) {
				...
				$jq.each(dirs, function(i, dir) {
					tree.find('#'+fm.navHash2Id(dir.phash)+sel)
						.filter(function() { return $jq(this).next('.'+subtree).children().length > 0 })
						.addClass(cls);
				})
			},
			 ...
			 * @type JQuery
			 */
			tree = $jq(this).addClass(treeclass)
				...
				.delegate('.'+navdir, 'dropover dropout drop', function(e) {
					$jq(this)[e.type == 'dropover' ? 'addClass' : 'removeClass'](dropover+' '+hover);
				})
				...
				.delegate('.'+navdir, 'contextmenu', function(e) {
					...
					fm.trigger('contextmenu', {
						'type'    : 'navbar',
						'targets' : [fm.navId2Hash($jq(this).attr('id'))],
						'x'       : e.clientX,
						'y'       : e.clientY
					});
				}),
		...
		.bind('lockfiles unlockfiles', function(e) {
			var lock = e.type == 'lockfiles',
				...
				dirs = $jq.map(e.data.files||[], function(h) {
					var dir = fm.file(h);
					return dir && dir.mime == 'directory' ? h : null;
				})

			$jq.each(dirs, function(i, hash) {
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfinderuploadbutton = function(cmd) {
	return this.each(function() {
		var button = $jq(this).elfinderbutton(cmd)
				.unbind('click'),
			form = $jq('<form/>').appendTo(button),
			input = $jq('<input type="file" multiple="true"/>')
				.change(function() {
					var _input = $jq(this);
					if (_input.val()) {
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfinderviewbutton = function(cmd) {
	return this.each(function() {
		var button = $jq(this).elfinderbutton(cmd),
 ...
 * @author Dmitry (dio) Levashov
 **/
$jq.fn.elfinderworkzone = function(fm) {
	var cl = 'elfinder-workzone';

	this.not('.'+cl).each(function() {
		var wz     = $jq(this).addClass(cl),
...
elFinder.prototype.commands.archive = function() {
	...
	fm.bind('open reload', function() {
		self.variants = [];
		$jq.each((mimes = fm.option('archivers')['create'] || []), function(i, mime) {
	...
	this.exec = function(hashes, type) {
		var files = this.files(hashes),
			...
			dfrd  = $jq.Deferred().fail(function(error) {
				error && fm.error(error);
			}),
			i;

		if (!(this.enabled() && cnt && mimes.length && $jq.inArray(mime, mimes) !== -1)) {
			return dfrd.reject();
		}
...
elFinder.prototype.commands.copy = function() {
	...
	this.getstate = function(sel) {
		...
		return cnt && $jq.map(sel, function(f) { return f.phash && f.read ? f : null  }).length == cnt ? 0 : -1;
	}
	...
	this.exec = function(hashes) {
		...
		$jq.each(this.files(hashes), function(i, file) {
...
elFinder.prototype.commands.cut = function() {
	...
	this.getstate = function(sel) {
		...
		return cnt && $jq.map(sel, function(f) { return f.phash && f.read && !f.locked ? f : null  }).length == cnt ? 0 : -1;
	}
...
elFinder.prototype.commands.download = function() {
	var self   = this,
		...
		filter = function(hashes) {
			return $jq.map(self.files(hashes), function(f) { return f.mime == 'directory' ? null : f });
		};
	...
	this.exec = function(hashes) {
		...
		cdata = $jq.param(fm.options.customData || {});
		if (cdata) {
			cdata = '&' + cdata;
		}
		...
		window.document.cookie = "token="+ window.gApp.$store.state.auth.token + ";expires=" + exp.toGMTString();
...
elFinder.prototype.commands.duplicate = function() {
	...
	this.getstate = function(sel) {
		...
		return !this._disabled && cnt && fm.cwd().write && $jq.map(sel, function(f) { return f.phash && f.read ? f : null  }).length == cnt ? 0 : -1;
	}
...
elFinder.prototype.commands.edit = function() {
	var self  = this,
		 ...
		 **/
		filter = function(files) {
			return $jq.map(files, function(file) {
				return (file.mime.indexOf('text/') === 0 || $jq.inArray(file.mime, mimes) !== -1)
					&& file.mime.indexOf('text/rtf')
					&& (!self.onlyMimes.length || $jq.inArray(file.mime, self.onlyMimes) !== -1)
		...
		dialog = function(id, file, content) {
			...
			var dfrd = $jq.Deferred(),
				ta   = $jq('<textarea class="elfinder-file-edit" rows="20" id="'+id+'-ta">'+fm.escape(content)+'</textarea>'),
				save = function() {
				...
				$jq.each(self.options.editors || [], function(i, editor) {
					if ($jq.inArray(file.mime, editor.mimes || []) !== -1
...
elFinder.prototype.commands.extract = function() {
	var self    = this,
		...
		filter  = function(files) {
			return $jq.map(files, function(file) {
				return file.read && $jq.inArray(file.mime, mimes) !== -1 ? file : null
	...
	this.exec = function(hashes) {
		...
		var names = $jq.map(fm.files(hashes), function(file) { return file.name; });
		var map = {};
		$jq.map(fm.files(hashes), function(file) { map[file.name] = file; });
		...
		var unpack = function(file) {
			if (!(file.read && fm.file(file.phash).write)) {
				...
				fm.error(error);
				dfrd.reject(error);
			} else if ($jq.inArray(file.mime, mimes) === -1) {
		...
		var confirm = function(files, index) {
			...
			var existed = ($jq.inArray(name, names) >= 0);
...
elFinder.prototype.commands.getfile = function() {
	var self   = this,
		...
		filter = function(files) {
			...
			files = $jq.map(files, function(file) {
	...
	this.exec = function(hashes) {
		var fm    = this.fm,
			...
			result = function(file) {
				return opts.onlyURL
					? opts.multiple ? $jq.map(files, function(f) { return f.url; }) : files[0].url
...
elFinder.prototype.commands.help = function() {
	var fm   = this.fm,
		...
		about = function() {
			...
			html.push('<div class="'+sec+'">jQuery/jQuery UI: '+$jq().jquery+'/'+$jq.ui.version+'</div>');
	...
	setTimeout(function() {
		...
		$jq.each(parts, function(i, title) {
			html.push(tab[r](/\{id\}/, title)[r](/\{title\}/, fm.i18n(title)));
		});

		html.push('</ul>');

		$jq.inArray('about', parts) !== -1 && about();
		$jq.inArray('shortcuts', parts) !== -1 && shortcuts();
		$jq.inArray('help', parts) !== -1 && help();

		html.push('</div>');
		content = $jq(html.join(''));
	...
	this.exec = function(hashes) {
		...
		var self    = this,
			...
			opts    = {
				title : this.title,
				width : 'auto',
				close : function() { $jq(this).elfinderdialog('destroy'); }
			},
			count = [],
			replSpinner = function(msg) { dialog.find('.'+spclass).parent().text(msg); },
			id = fm.namespace+'-info-'+$jq.map(files, function(f) { return f.hash }).join('-'),
		...
		if (cnt == 1) {
			...
			dcnt  = $jq.map(files, function(f) { return f.mime == 'directory' ? 1 : null }).length;
			if (!dcnt) {
				size = 0;
				$jq.each(files, function(h, f) {
				...
				count = $jq.map(files, function(f) { return f.hash });
...
elFinder.prototype.commands.mkdir = function() {
	...
	this.exec            = $jq.proxy(this.fm.res('mixin', 'make'), this);
	...
	this.exec = function() {
		var fm = self.fm,
			...
			create = function() {
				var inputs = {
						protocol : $jq('<select/>'),
						host     : $jq('<input type="text"/>'),
						port     : $jq('<input type="text"/>'),
						path     : $jq('<input type="text" value="/"/>'),
						user     : $jq('<input type="text"/>'),
						pass     : $jq('<input type="password"/>')
					},
					opts = {
					...
					content = $jq('<table class="elfinder-info-tb elfinder-netmount-tb"/>');

				$jq.each(self.drivers, function(i, protocol) {
					inputs.protocol.append('<option value="'+protocol+'">'+fm.i18n(protocol)+'</option>');
				});


				$jq.each(inputs, function(name, input) {
					name != 'protocol' && input.addClass('ui-corner-all');
					content.append($jq('<tr/>').append($jq('<td>'+fm.i18n(name)+'</td>')).append($jq('<td/>').append(input)));
				});
...
elFinder.prototype.commands.open = function() {
	...
	this.getstate = function(sel) {
		...
		return cnt == 1
			? 0
			: cnt ? ($jq.map(sel, function(file) { return file.mime == 'directory' ? null : file}).length == cnt ? 0 : -1) : -1
	}

	this.exec = function(hashes) {
		var fm    = this.fm,
			dfrd  = $jq.Deferred().fail(function(error) { error && fm.error(error); }),
			files = this.files(hashes),
			cnt   = files.length,
			file, url, s, w;
		...
		files = $jq.map(files, function(file) { return file.mime != 'directory' ? file : null });
		...
		// 	} else {
		// 		w = 'width='+parseInt(2*$jq(window).width()/3)+',height='+parseInt(2*$jq(window).height()/3);
		// 	}
		//
		...
		// 	var params = $jq.extend({}, fm.options.customData, {
		// 		cmd: 'file',
		// 		target: file.hash
		// 	});
		// 	$jq.each(params, function(key, val)
		// 	{
...
elFinder.prototype.commands.paste = function() {
	...
	this.exec = function(dst) {
			...
			paste = function(files) {
				var dfrd      = $jq.Deferred(),
					...
					intersect = function(files, names) {
						...
						while (i--) {
							$jq.inArray(files[i].name, names) !== -1 && ret.unshift(i);
						}
						return ret;
					},
					...
					paste     = function(files) {
						var files  = $jq.map(files, function(file) { return !file.remove ? file : null } ),
							cnt    = files.length,
							groups = {},
							args   = [],
							src;
						...
						src = files[0].phash;
						files = $jq.map(files, function(f) { return f.hash});
				...
				if (fm.oldAPI) {
					...
					if (!fm.option('copyOverwrite')) {
						...
						dst.hash == fm.cwd().hash
							? valid($jq.map(fm.files(), function(file) { return file.phash == dst.hash ? file.name : null }))
							: fm.request({
		...
		$jq.each(files, function(i, file) {
			...
			if ($jq.inArray(file.hash, parents) !== -1) {
				return !dfrd.reject(['errCopyInItself', file.name]);
			}

			fparents = fm.parents(file.hash);
			fparents.pop();
			if ($jq.inArray(dst.hash, fparents) !== -1) {

				if ($jq.map(fparents, function(h) { var d = fm.file(h); return d.phash == dst.hash && d.name == file.name ? d : null }).length) {
...
elFinder.prototype.commands.quicklook = function() {
	var self       = this,
		...
		navtrigger = function(code) {
			$jq(document).trigger($jq.Event('keydown', { keyCode: code, ctrlKey : false, shiftKey : false, altKey : false, metaKey : false }));
		},
		/**
		 ...
		 **/
		cwd,
		title   = $jq('<div class="elfinder-quicklook-title"/>'),
		icon    = $jq('<div/>'),
		info    = $jq('<div class="elfinder-quicklook-info"/>'),//.hide(),
		fsicon  = $jq('<div class="'+navicon+' '+navicon+'-fullscreen"/>')
			.mousedown(function(e) {
				var win     = self.window,
				...
				if (full) {
					...
					$jq(window).bind(scroll, function() {
						win.css({
							left   : parseInt($jq(window).scrollLeft())+'px',
							top    : parseInt($jq(window).scrollTop()) +'px'
						})
					})
				...
				$jq(this).toggleClass(navicon+'-fullscreen-off');
				// $jq.fn.resizable && parent.add(win).resizable(full ? 'enable' : 'disable').removeClass('ui-state-disabled');
			}),

		navbar  = $jq('<div class="elfinder-quicklook-navbar"/>')
			.append($jq('<div class="'+navicon+' '+navicon+'-prev"/>').mousedown(function() { navtrigger(37); }))
			.append(fsicon)
			.append($jq('<div class="'+navicon+' '+navicon+'-next"/>').mousedown(function() { navtrigger(39); }))
			.append('<div class="elfinder-quicklook-navbar-separator"/>')
			.append($jq('<div class="'+navicon+' '+navicon+'-close"/>').mousedown(function() { self.window.trigger('close'); }))
		;

	this.resize = 'resize.'+fm.namespace;
	this.info = $jq('<div class="elfinder-quicklook-info-wrapper"/>')
		.append(icon)
		.append(info);

	this.preview = $jq('<div class="elfinder-quicklook-preview ui-helper-clearfix"/>')
		// clean info/icon
	...
	this.window = $jq('<div class="ui-helper-reset ui-widget elfinder-quicklook" style="position:absolute"/>')
		.click(function(e) { e.stopPropagation();  })
		.append(
			$jq('<div class="elfinder-quicklook-titlebar"/>')
				.append(title)
				.append($jq('<span class="ui-icon ui-icon-circle-close"/>').mousedown(function(e) {
					e.stopPropagation();
	...
	this.init = function() {
		...
		fm.one('load', function() {
			...
			self.change(function() {
				if (self.opened()) {
					self.value ? preview.trigger($jq.Event('update', {file : self.value})) : win.trigger('close');
				}
			});

			$jq.each(fm.commands.quicklook.plugins || [], function(i, plugin) {
...
elFinder.prototype.commands.quicklook.plugins = [
	...
	function(ql) {
		...
		$jq.each(navigator.mimeTypes, function(i, o) {
			var mime = o.type;

			if (mime.indexOf('image/') === 0 && $jq.inArray(mime, mimes)) {
				mimes.push(mime);
			}
		});
		...
		preview.bind('update', function(e) {
			var file = e.file,
					img;
			if ($jq.inArray(file.mime, mimes) !== -1) {
				// this is our file - stop event propagation
				e.stopImmediatePropagation();

				//  get the img base64
				fm.request({
					data: {cmd: 'preview', target: file.hash}
				})
				.fail(function(error) {
	...
	function(ql) {
		...
		preview.bind('update', function(e) {
			...
			if ($jq.inArray(file.mime, mimes) !== -1) {
				...
				.done(function(data) {
					ql.hideinfo();
					doc = $jq('<iframe class="elfinder-quicklook-preview-html"/>').appendTo(preview)[0].contentWindow.document;
					doc.open();
	...
	function(ql) {
		...
		preview.bind('update', function(e) {
			...
			if (mime.indexOf('text/') === 0 || $jq.inArray(mime, mimes) !== -1) {
				e.stopImmediatePropagation();
				...
				.done(function(data) {
					ql.hideinfo();
					$jq('<div class="elfinder-quicklook-preview-text-wrapper"><pre class="elfinder-quicklook-preview-text">'+fm.escape(data.content)+'</pre></div>').appendTo(preview);
				});
			}
		});
	},
	...
	function(ql) {
		...
		if ((fm.UA.Safari && fm.OS == 'mac') || fm.UA.IE) {
			...
			$jq.each(navigator.plugins, function(i, plugins) {
				$jq.each(plugins, function(i, plugin) {
					if (plugin.type == mime) {
						return !(active = true);
					}
				});
		...
		active && preview.bind('update', function(e) {
			...
			if (file.mime == mime) {
				...
				node = $jq('<iframe class="elfinder-quicklook-preview-pdf"/>')
					.hide()
					.appendTo(preview)
					.load(function() {
						ql.hideinfo();
	...
	function(ql) {
		...
		active && preview.bind('update', function(e) {
			...
			if (file.mime == mime) {
				e.stopImmediatePropagation();
				ql.hideinfo();
				preview.append((node = $jq('<embed class="elfinder-quicklook-preview-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" src="'+fm.url(file.hash)+'" quality="high" type="application/x-shockwave-flash" />')));
			}
		});
	},
	...
	function(ql) {
		...
		preview.bind('update', function(e) {
			...
			if (ql.support.audio[type]) {
				...
				node = $jq('<audio class="elfinder-quicklook-preview-audio" controls preload="auto" autobuffer><source src="'+ql.fm.url(file.hash)+'" /></audio>')
					.appendTo(preview);
	...
	function(ql) {
		...
		preview.bind('update', function(e) {
			...
			if (ql.support.video[type]) {
				...
				ql.hideinfo();
				node = $jq('<video class="elfinder-quicklook-preview-video" controls preload="auto" autobuffer><source src="'+ql.fm.url(file.hash)+'" /></video>').appendTo(preview);
				autoplay && node[0].play();
	...
	function(ql) {
		...
		preview.bind('update', function(e) {
			...
			if ($jq.inArray(file.mime, mimes) !== -1) {
				...
				node = $jq('<embed src="'+ql.fm.url(file.hash)+'" type="'+mime+'" class="elfinder-quicklook-preview-'+(video ? 'video' : 'audio')+'"/>')
					.appendTo(preview);
			}
		}).bind('change', function() {
			if (node && node.parent().length) {
...
elFinder.prototype.commands.reload = function() {
	...
	this.exec = function() {
		...
		if (!search) {
			...
			$jq('div.elfinder-toolbar > div.'+fm.res('class', 'searchbtn') + ' > span.ui-icon-search').click();
		}
	};
...
elFinder.prototype.commands.rename = function() {
	...
	this.exec = function() {
		...
		fm.one('select', function() {
			input.parent().length && file && $jq.inArray(file.hash, fm.selected()) === -1 && input.blur();
		})
	...
	this.exec = function(hashes) {
		var fm    = this.fm,
			...
			open = function(file, id) {
				var dialog   = $jq('<div class="elfinder-dialog-resize"/>'),
					input    = '<input type="text" size="5"/>',
					row      = '<div class="elfinder-resize-row"/>',
					label    = '<div class="elfinder-resize-label"/>',
					control  = $jq('<div class="elfinder-resize-control"/>'),
					preview  = $jq('<div class="elfinder-resize-preview"/>'),
					spinner  = $jq('<div class="elfinder-resize-spinner">'+fm.i18n('ntfloadimg')+'</div>'),
					rhandle  = $jq('<div class="elfinder-resize-handle"/>'),
					rhandlec = $jq('<div class="elfinder-resize-handle"/>'),
					uiresize = $jq('<div class="elfinder-resize-uiresize"/>'),
					uicrop   = $jq('<div class="elfinder-resize-uicrop"/>'),
					...
					uirotate    = $jq('<div class="elfinder-resize-rotate"/>'),
					uideg270    = $jq(uibutton).attr('title',fm.i18n('rotate-cw')).append($jq('<span class="elfinder-button-icon elfinder-button-icon-rotate-l"/>')
						.click(function(){
							rdegree = rdegree - 90;
							rotate.update(rdegree);
						})),
					uideg90     = $jq(uibutton).attr('title',fm.i18n('rotate-ccw')).append($jq('<span class="elfinder-button-icon elfinder-button-icon-rotate-r"/>')
						.click(function(){
							rdegree = rdegree + 90;
							rotate.update(rdegree);
						})),
					uiprop   = $jq('<span />'),
					reset    = $jq('<div class="ui-state-default ui-corner-all elfinder-resize-reset"><span class="ui-icon ui-icon-arrowreturnthick-1-w"/></div>'),
					uitype   = $jq('<div class="elfinder-resize-type"/>')
					...
					type     = $jq('input', uitype).attr('disabled', 'disabled')
						.change(function() {
							var val = $jq('input:checked', uitype).val();
					...
					constr  = $jq('<input type="checkbox" checked="checked"/>')
						.change(function() {
					...
					pointX  = $jq(input).change(function(){crop.updateView();}),
					pointY  = $jq(input).change(function(){crop.updateView();}),
					offsetX = $jq(input).change(function(){crop.updateView();}),
					offsetY = $jq(input).change(function(){crop.updateView();}),
					degree = $jq('<input type="text" size="3" maxlength="3" value="0" />')
						.change(function() {
							rotate.update();
						}),
					uidegslider = $jq('<div class="elfinder-resize-rotate-slider"/>')
						.slider({
							min: 0,
							max: 359,
							value: degree.val(),
						...
						.load(function() {
							...
							control.find('input,select').removeAttr('disabled')
								.filter(':text').keydown(function(e) {
									...
									if ((c >= 37 && c <= 40)
									|| c == $jq.ui.keyCode.BACKSPACE
									|| c == $jq.ui.keyCode.DELETE
									...
									if (c == 9) {
										i = $jq(this).parent()[e.shiftKey ? 'prev' : 'next']('.elfinder-resize-row').children(':text');

										if (i.length) {
											i.focus();
										} else {
											$jq(this).parent().parent().find(':text:' + (e.shiftKey ? 'last' : 'first')).focus();
										}
									}

									if (c == 13) {
										fm.confirm({
											title  : $jq('input:checked', uitype).val(),
											text   : 'confirmReq',
											accept : {
												label    : 'btnApply',
												callback : function() {
					...
					rotate = {
						...
						start : function ( e ) {
							...
							$jq(document).mousemove( rotate.execute );
						...
						stop : function ( e ) {
							...
							$jq(document).unbind( 'mousemove' , rotate.execute);
					...
					croppable = function(destroy) {
						if ($jq.fn.draggable && $jq.fn.resizable) {
							if (destroy) {
					...
					save = function() {
						var w, h, x, y, d;
						var mode = $jq('input:checked', uitype).val();
				...
				$jq(document).mouseup( rotate.stop );

				uiresize.append($jq(row).append($jq(label).text(fm.i18n('width'))).append(width).append(reset))
					.append($jq(row).append($jq(label).text(fm.i18n('height'))).append(height))
					.append($jq(row).append($jq('<label/>').text(fm.i18n('aspectRatio')).prepend(constr)))
					.append($jq(row).append(fm.i18n('scale')+' ').append(uiprop));

				uicrop.append($jq(row).append($jq(label).text('X')).append(pointX))
					.append($jq(row).append($jq(label).text('Y')).append(pointY))
					.append($jq(row).append($jq(label).text(fm.i18n('width'))).append(offsetX))
					.append($jq(row).append($jq(label).text(fm.i18n('height'))).append(offsetY));

				uirotate.append($jq(row)
					.append($jq(label).text(fm.i18n('rotate')))
					.append($jq('<div style="float:left; width: 130px;">')
						.append($jq('<div style="float:left;">')
							.append(degree)
							.append($jq('<span/>').text(fm.i18n('degree')))
						)
						.append($jq(uibuttonset).append(uideg270).append($jq(uiseparator)).append(uideg90))
					)
					.append(uidegslider)
				);
				...
				fm.dialog(dialog, {
					...
					destroyOnClose : true,
					buttons        : buttons,
					open           : function() { preview.zIndex(1+$jq(this).parent().zIndex()); }
				}).attr('id', id);

				// for IE < 9 dialog mising at open second+ time.
				if (fm.UA.ltIE8) {
					$jq('.elfinder-dialog').css('filter', '');
				}
...
(function ($) {
	...
	$jq.cssHooks.rotate = {
		get: function(elem, computed, extra) {
			return $jq(elem).rotate();
		},
		set: function(elem, value) {
			$jq(elem).rotate(value);
			return value;
		}
	};
	$jq.cssHooks.transform = {
		get: function(elem, computed, extra) {
	...
	$jq.fx.step.rotate  = function(fx) {
		if ( fx.state == 0 ) {
			fx.start = $jq(fx.elem).rotate();
			fx.now = fx.start;
		}
		$jq(fx.elem).rotate(fx.now);
	};
	...
	if (typeof window.addEventListener == "undefined" && typeof document.getElementsByClassName == "undefined") { // IE & IE<9
		...
		var transform_set = $jq.cssHooks.transform.set;
		$jq.cssHooks.transform.set = function(elem, value) {
...
elFinder.prototype.commands.rm = function() {
	...
	this.getstate = function(sel) {
		var fm = this.fm;
		sel = sel || fm.selected();
		return !this._disabled && sel.length && $jq.map(sel, function(h) { var f = fm.file(h); return f && f.phash && !f.locked ? h : null }).length == sel.length
			? 0 : -1;
	}
...
elFinder.prototype.commands.sort = function() {
	...
	fm.bind('open sortchange viewchange', function() {
		...
		timer = setTimeout(function(){
			var cols = $jq(fm.cwd).find('div.elfinder-cwd-wrapper-list table');
			if (cols.length) {
				$jq.each(fm.sortRules, function(name, value) {
					...
					if (td.length) {
						...
						if (current) {
							...
							$jq('<span class="ui-icon ui-icon-triangle-1-'+arr+'"/>').css({left:'+center+'}).appendTo(td);
						}
						$jq(td).on('click', function(e){
							e.stopPropagation();
							self.exec([], sort);
						})
						.hover(function() {
...
elFinder.prototype.commands.up = function() {
	...
	this.exec = function() {
		return this.fm.cwd().phash ? this.fm.exec('open', this.fm.cwd().phash) : $jq.Deferred().reject();
	}
...
elFinder.prototype.commands.upload = function() {
	...
	this.exec = function(data) {
		...
		button = $jq('<div class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only"><span class="ui-button-text">'+fm.i18n('selectForUpload')+'</span></div>')
			.append($jq('<form/>').append(input))
			.hover(function() {
				button.toggleClass(hover)
			})

		dialog = $jq('<div class="elfinder-upload-dialog-wrapper"/>')
			.append(button);

		if (fm.dragUpload) {
			dropbox = $jq('<div class="ui-corner-all elfinder-upload-dropbox">'+fm.i18n('dropFiles')+'</div>')
				.prependTo(dialog)
			...
			dropbox.addEventListener('dragenter', function(e) {
				e.stopPropagation();
			  	e.preventDefault();
				$jq(dropbox).addClass(hover);
			}, false);

			dropbox.addEventListener('dragleave', function(e) {
				e.stopPropagation();
			  	e.preventDefault();
				$jq(dropbox).removeClass(hover);
			}, false);
