...
def test_sql_template(policy):
    ...
    expect = "select {columns} from " \
             "\"hour\".node_gpu_mem where time > now() - 80s "
    assert sql == expect


@mark.django_db
def test_get_mem_data(policy, mocker):
    mock = mocker.patch("django.core.cache.cache.get")
    DataSource(policy)._get_gpu_mem()
