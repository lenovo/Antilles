...
def test_sql_template(policy):
    ...
    expect = "select {columns} from " \
             "\"hour\".node_energy where time > now() - 80s "
    assert sql == expect


@mark.django_db
def test_get_energy_data(policy, mocker):
    mock = mocker.patch("django.core.cache.cache.get")
    DataSource(policy)._get_energy()

    mock.assert_called_with(
        "select host, min(value) as val from "
        "\"hour\".node_energy where time > now() - 80s "
        "group by host",
        epoch='s'
    )
