...
def policy():
    ...
    return Policy.objects.create(
        metric_policy=Policy.GPU_TEMP,
        name='policy_gpu_temp',
        portal='{ "$gt" : 70}',
...
def test_sql_template(policy):
    ...
    expect = "select {columns} from " \
             "\"hour\".node_gpu_temp where time > now() - 80s "
    assert sql == expect
...
def test_get_temp_data(policy, mocker):
    ...
    DataSource(policy)._get_gpu_temp()

    mock.assert_called_with(
        "select host, min(value) as val, index from "
        "\"hour\".node_gpu_temp where time > now() - 80s group by host",
        epoch='s'
    )
